...
async function init() {
  console.log(
    chalk.green(
      "Welcome to the codetie (Sync swift + xcode) Project Setup 🚀\n"
    )
  );
    ...
    if (fs.readdirSync(root).length === 0) {
      console.log(
        `Directory ${chalk.cyan(
          root
        )} is empty. Proceeding with project creation.`
      );
      debugLog(`Directory ${root} is empty. Proceeding with project creation.`);
    } else {
      console.log(
        chalk.red(
          `Error: Directory ${root} is not empty. Please choose a different directory or empty it.`
        )
      );
  ...
  for (const file of mustacheFiles) {
    ...
    if (fs.existsSync(srcPath)) {
      ...
      if (fs.existsSync(targetMustacheFile)) {
        ...
        debugLog(
          `Removed Mustache file from target directory: ${targetMustacheFile}`
        );
      }
    } else {
  ...
  if (generateProject) {
    ...
    try {
      ...
      if (fs.existsSync(oldMainSwift)) {
        ...
        mainSwiftContent = mainSwiftContent.replace(
          "HelloWorldApp",
          `${targetDir}App`
        );
        ...
        debugLog(
          `Renamed and updated _main.swift: ${oldMainSwift} -> ${newMainSwift}`
        );
      }
      ...
      if (startSimulator) {
        ...
        console.log(root);
        try {
          execSync(`bash scripts/build_project.sh ${targetDir}`, {
            cwd: root,
            stdio: "inherit",
          });
        ...
        try {
          execSync("bash scripts/run_simulator.sh", {
            cwd: root,
            stdio: "inherit",
          });
          console.log(
          ...
          console.error(
            chalk.red("\n❌ Failed to start project in simulator:"),
            error
          );
        }
      }
