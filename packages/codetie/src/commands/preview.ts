  ...
  async run(ctx) {
    ...
    try {
      ...
      const appPath = resolve(
        cwd,
        `build/Build/Products/Debug-iphonesimulator/${config.project_name}.app`
      )
      ...
      const deviceObject = destr<{ devices: { [key: string]: ISimulator[] } }>(
        `${stdout}`
      )
      ...
      const selectedSimulator = await consola.prompt(
        'Select an iOS simulator to start:',
        {
          type: 'select',
          required: true,
      ...
      const simulatorId = (
        simulators.find(
          (simulator) => simulator.name === selectedSimulator
        ) as ISimulator
      ).udid
      ...
      while (
        (
          await $(
            `xcrun simctl list devices | grep "${simulatorId}" | grep -o "Booted"`,
            [],
            { shell: true }
          )
        ).stdout.trim() !== 'Booted'
      ...
      await $(
        `xcrun simctl launch booted "${config.bundle_id_prefix}.${config.project_name}"`,
        [],
        { shell: true }
      )
